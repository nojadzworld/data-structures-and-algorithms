...
const toTitleCase = (arr) => {
  const titleCaseArr = arr.map(str => {
...
let biggerThanLuke = (arr) => {
  ...
  const biggerCharacters = arr.filter((character) => parseInt(character.mass) > parseInt(luke.mass) && character.name !== 'Luke Skywalker');
...
const isSecure = (url) => {
  ...
  return colonIndex > -1 && colonIndex < firstSlashIndex && url.startsWith('https://');
};
